...
extension UIImageView {
    ...
    public func setImage(string: String?,
        ...
        let image = imageSnap(text: string != nil ? string?.initials : "",
                              ...
                              circular: circular,
                              stroke: stroke,
                              textAttributes: textAttributes, sizee: sizee, backColor: backColor, fontColor: fontColor)
    ...
    private func imageSnap(text: String?,
        ...
        // FillAppColors.gradientPoint2!.cgColor
        context?.setFillColor(backColor)
        context?.fill(CGRect(x: 0, y: 0, width: size.width, height: size.height))
        
        let attributes = textAttributes ?? [NSAttributedString.Key.foregroundColor: fontColor,
                                            NSAttributedString.Key.font: UIFont(name: "AvenirNextLTPro-Demi", size: sizee) ?? UIFont()]
        // stroke color
        if stroke {
            // outer circle
